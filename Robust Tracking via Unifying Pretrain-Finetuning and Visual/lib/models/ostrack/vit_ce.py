...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        patch_size_s = to_2tuple(patch_size)
        val = math.sqrt(6. / float(3 * reduce(mul, patch_size_s, 1) + prompt_dim))  # noqa
        self.prompt_embeddings = nn.Parameter(torch.zeros(
            1, num_prompt, prompt_dim))
        nn.init.uniform_(self.prompt_embeddings.data, -val, val)
        transformer_num_layers = 12
        total_d_layer = transformer_num_layers - 1
        self.deep_prompt_embeddings = nn.Parameter(torch.zeros(
            total_d_layer, num_prompt, prompt_dim))
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        lens_z = self.pos_embed_z.shape[1] + self.num_prompt
        ...
        for i in range(num_layers):
            if i == 0:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    self.blocks[i](x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
            else:
                x = torch.cat((
                    self.prompt_dropout(
    ...
    def Freeze(self):
        ...
        self.pos_embed_x.requires_grad = True
        self.pos_embed_z.requires_grad = True
        self.cls_token.requires_grad = True
        self.pos_embed.requires_grad = True
        frozen_layer =  ['norm.weight','norm.bias']
        freeze = ['mlp','attn.qkv.bias']
        if any(freeze):
            for k, v in self.named_parameters():
                if k in frozen_layer:
