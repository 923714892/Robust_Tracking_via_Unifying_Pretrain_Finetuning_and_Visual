...
class OSTrack(BaseTracker):
    ...
    def track(self, image,seq,frame_num, info: dict = None):
        H, W, _ = image.shape
        self.frame_id += 1
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                save_path = f'/home/local_data/zgt/OSTrack-main/{seq}'
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                cv2.imwrite(os.path.join(save_path, "%04d.jpg" % frame_num), image_BGR)
            else:
